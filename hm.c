...
struct number *do_sub (struct number *x, struct number *y) {
  struct number *m_y=do_scale(y,1), *res;
...
struct number *do_scale (struct number* num, int x) {
  ...
  tmp=do_scale(num,x-1);
  result=do_add(num,tmp);
  destroy_number(tmp);
...
struct number *do_div (struct number * x, struct number *y) {
  ...
  for (cx=x->next; cx != x; cx=cx->next) {
    ...
    while (compare_abs(divisor, abs_y) != DIGITAL_LT ) {
      dz++;
      tmp_divisor=do_sub(divisor,abs_y);
...
struct number *do_remain (struct number * x, struct number *y) {
...
int main() {
  ...
  write_down(do_div(x,y));
  write_down(do_remain(x,y));
